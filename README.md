...
openai의 논문과 같은 방식으로 보상은 한가지 시야에 들었으냐 유무로 진행했다 따라서 두 에이전트의 목표는 다음과 같다고 할 수 있다.
- **Ghost의 목표**: Pacman을 자신의 시야 안에 포착하기
- **Pacman의 목표**: Ghost의 시야에 들키지 않고 최대한 오래 생존하기

사람이 규칙을 직접 짜넣는 대신, 에이전트들이 수천 번의 게임을 반복하면서 **스스로** 데이터를 형성해 추격·포위·도주 전략을 발견하도록 학습시켰다.
...
```mermaid
graph LR
    GS["전역 상태Ghost1, Ghost2, Pacman 위치 전부"] -->|"Centralized Training학습 시"| C["Critic가치 추정"]
    LO["로컬 관측자기 레이더 정보만"] -->|"Decentralized Execution실행 시"| A["Actor행동 결정"]
```
...
update_env=8을 채택해 update 1회에 같은 전략을 가진 환경 8개를 병렬로 돌려 데이터를 얻게했다. 
...
```mermaid
flowchart TD
    LOOP["🔁 학습 루프 — 1500 업데이트 반복"]

    LOOP --> R["1. collect_rollout_parallel()"]
    R --> R1["64개의 환경을 병렬로 실행"]
    R --> R2["128 스텝 동안 obs, action, reward, done 수집"]
    R --> R3["GAE로 Advantage & Return 계산"]

    LOOP --> U["2. ppo_update()"]
    U --> U1["epoch=4 반복"]
    U --> U2["mini-batch로 나누어 그래디언트 계산"]
    U --> U3["PPO Clip Loss + Value Loss + Entropy Bonus"]
    U --> U4["Adam Optimizer로 파라미터 업데이트"]

    LOOP --> S["3. 체크포인트 저장 (50 업데이트마다)"]
    S --> S1["Google Drive에 .pt 파일 저장"]
```
