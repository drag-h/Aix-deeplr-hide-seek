...
| 한계 | 내용 |
|---|---|
| 시각화 수준 | 텍스트(ASCII) 렌더링만 지원. Matplotlib/Pygame 기반 GUI 미구현 |
| 고정된 에이전트 수 | Ghost 2마리로 고정. 가변 에이전트 수 미지원 |
| 학습 곡선 자동화 | reward 그래프 자동 저장 미구현 |
| 맵 다양성 | 3종 맵 외에 동적 생성 맵 미지원 |

### 향후 개선 방향

1. **GUI 시각화**: Pygame으로 실시간 렌더링 구현 → 직관적인 행동 분석 가능
2. **가변 에이전트**: Ghost/Pacman 수를 파라미터로 조정 가능하도록 확장
3. **보상 커리큘럼**: 쉬운 환경에서 점진적으로 어려운 환경으로 이동하는 커리큘럼 학습
4. **자동 평가 지표**: 생존 스텝 수, 포획 성공률 등 정량 지표 자동 기록
5. **Communication 채널**: Ghost 에이전트 간 명시적 통신 채널 추가 (CommNet, QMIX 등)
